...
export default function User({client, consents, policy}) {
    return <div>
        ...
        <Policy clientID={client.clientID} consents={consents} policy={policy} orgId={1}/>
    </div>
...
export async function getStaticPaths() {
    ...
    const consents = await prisma.consent.findMany({where: {orgReference: 1}})
...
export async function getStaticProps({params: {user}}) {
    ...
    const consents = await prisma.consent.findMany({where: {subjectId: parseInt(user), orgReference: 1}})
