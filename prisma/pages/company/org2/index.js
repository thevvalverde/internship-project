...
export default function Org2({orgInfo, subjectsUnique}) {
    return <div>
        <h1>This is company 2</h1>
        <p>Our contact phone is {orgInfo.contactPhone}</p>
        <p>Our contact email is {orgInfo.contactEmail}</p>
        <h1>Who are you?</h1>
        <ul>
            {subjectsUnique.map(subject => <li key={subject.clientID}><Link href={"/company/org2/"+subject.clientID}><a>{subject.name}</a></Link></li>)
            }
        </ul>
    </div>
}
...
export async function getStaticProps() {
    const orgInfo  = await prisma.organization.findUnique({where: {organizationID: 2}})
